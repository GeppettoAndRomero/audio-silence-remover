...
test.describe('i18n', () => {
  ...
  for (const loc of [
    { path: '/audio-silence-remover/', lang: 'en' },
    { path: '/audio-silence-remover/ja/', lang: 'ja' },
  ]) {
  ...
  test('serves every locale with the correct <html lang>', async ({ page }) => {
    const expected: Array<[string, string]> = [
      ['/audio-silence-remover/', 'en'],
      ['/audio-silence-remover/ja/', 'ja'],
      ['/audio-silence-remover/zh/', 'zh-Hans'],
      ['/audio-silence-remover/de/', 'de'],
      ['/audio-silence-remover/es/', 'es'],
    ];
